...
min_height = 0 
max_height = img_arr.shape[0]   # Get height (first dimension)

min_width = 0
max_width = img_arr.shape[1]    # Get width (second dimension)   
...
crop_arr = img_arr[crop_min_h:crop_max_h, crop_min_w:crop_max_w]  # Use NumPy array slicing


# ----- Displaying the cropped image and creating a download button to download the image -----

if type(crop_arr) == np.ndarray:
    st.image(crop_arr, caption="Cropped Image", width=500)
